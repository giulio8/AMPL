...
# ---------- PAYOFF PARAMETERS ----------------------------------------------
param RD_capture{TARGETS};                # defender reward if capture
param RA_capture{TARGETS};                # attacker reward if capture
param RD_no_capture{TARGETS};                # defender reward if not capture
param RA_no_capture{TARGETS};                # attacker reward if not capture



param MATTACK :=
      max {(r,c) in TARGETS}
          ( max(abs(RA_capture[r,c]), abs(RA_no_capture[r,c])) );
...
s.t. Define_u {(rt, ct) in TARGETS, t in T1}:
      u_theta[rt, ct,t] =
            p[rt, ct,t]   * RA_capture[rt, ct]
          + (1 - p[rt, ct,t]) * RA_no_capture[rt, ct];
...
s.t. DefenderUtil:
      UD = sum{(rt, ct) in TARGETS, t in T1}
             attack[rt, ct,t] *
             (   p[rt, ct,t]   * RD_capture[rt, ct]
               + (1 - p[rt, ct,t]) * RD_no_capture[rt, ct] );
